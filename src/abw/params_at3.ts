...
export type Bit_offsets = Record<
  string,
  { desc: string; bit_offset: number; bit_length: number; unit?: string }
>;
...
export function u32_to_i32(u32: number): number {
  if (u32 < U32_MIN || u32 > U32_MAX) {
    throw new Error(
      "the value is out of the range of 32 bit unsigned integers",
    );
  } else {
...
export function parse_txt_as_i32(txt: string): {
  i32_num: number;
  u32_hex: string;
} {
  if (REGEXP_DEC_NUM_STR.test(txt)) {
    ...
    throw new Error(
      "invalid number format, must be either an unsigned hexadecimal or signed decimal 32 bit integer",
    );
  }
}
...
export const NOTIFICATIONS: Option_ids = {
  ...
  geozone_out_hazard: {
    id: 0x0503,
    desc: "Exit from a hazardous area detected",
  },
...
export const EVENTS: Option_ids = {
  ...
  btn1_simple_seq: { id: 0x0005, desc: "Button1 simple sequence" },
  btn1_seq_reset: { id: 0x0006, desc: "Button1 reset sequence" },
  btn1_seq_bond_del: { id: 0x0007, desc: "Button1 bond delete sequence" },
  btn1_seq_ble_boot: { id: 0x0008, desc: "Button1 BLE bootloader sequence" },
  btn1_ble_connectivity: {
    id: 0x0009,
    desc: "Button1 event to start connectivity",
  },
  ...
  btn2_simple_seq: { id: 0x0005, desc: "Button2 simple sequence" },
  btn2_seq_reset: { id: 0x0106, desc: "Button2 reset sequence" },
  btn2_seq_bond_del: { id: 0x0107, desc: "Button2 bond delete sequence" },
  btn2_seq_ble_boot: { id: 0x0108, desc: "Button2 BLE bootloader sequence" },
  btn2_ble_connectivity: {
    id: 0x0109,
    desc: "Button2 event to start connectivity",
  },

  buzzer_on: { id: 0x0200, desc: "The buzzer is playing" },
  buzzer_off: { id: 0x0201, desc: "The buzzer is no more playing" },

  acc_motion_start: { id: 0x0300, desc: "Motion Start event detected" },
  acc_motion_end: { id: 0x0301, desc: "Motion End event detected" },
  acc_shock_detected: { id: 0x0302, desc: "Shock Event detected" },
  acc_clear_motion: { id: 0x0303, desc: "Clear Motion event received" },

  power_battery_low: {
    id: 0x0400,
    desc: "Critically low battery level detected",
  },
  ...
  temp_update: { id: 0x0503, desc: "Temperature updated" },
  ...
  network_cellular_lpm_changed: {
    id: 0x0807,
    desc: "Cellular network lpm changed",
  },
  ...
  core_hold: { id: 0x0904, desc: "Core App went to Hold State" }, // The core application moved to the hold state. Reserved for future use.,
  core_time_update: { id: 0x0905, desc: "System time updated" },
  core_device_start: {
    id: 0x0906,
    desc: "Core App Start action requested",
  },
  core_device_stop: {
    id: 0x0907,
    desc: "Core App Stop action requested",
  },
  core_sos_start: { id: 0x0908, desc: "SOS Start action requested" },
  core_sos_stop: { id: 0x0909, desc: "SOS Stop action requested" },
  core_status_ul: { id: 0x090a, desc: "Status Uplink action requested" }, // The sending of a status uplink has been requested,
  core_monitoring: { id: 0x090b, desc: "Monitoring timeout" }, // The timeout of the monitoring timer has elapsed,
  core_pod: { id: 0x090c, desc: "Position on Demand action requested" },
  core_refresh_alm: { id: 0x090d, desc: "Almanac refresh" },
  core_tamper_on: { id: 0x090e, desc: "Tacker casing is open" },
  core_tamper_off: { id: 0x090f, desc: "Tacker casing is closed" },
  core_gnss_timeout: { id: 0x0910, desc: "GNSS core timeout" },
  core_battery_level: { id: 0x0911, desc: "Show battery level" },
  core_vbus_on: { id: 0x0912, desc: "VBUS connected" },
  core_vbus_off: { id: 0x0913, desc: "VBUS disconnected" },
  core_dl_trigger: {
    id: 0x0914,
    desc: "HeartBeat message sent",
  },
  core_join_to_activate‍: {
    id: 0x0915,
    desc: "Automatic join procedure started", // This event is sent when the tracker must join LoRa before being activated (meaning fully running).,
  },
  ...
  ble_conn_link_lost: { id: 0x0a06, desc: "BLE connection lost" },
  ble_bonded: { id: 0x0a07, desc: "BLE is bonded" },
  ble_beacon_start: { id: 0x0a08, desc: "BLE beacon started" },
  ble_beacon_stop: { id: 0x0a09, desc: "BLE beacon stopped" },
  ble_reduce_cnx_speed_rqst: {
    id: 0x0a0a,
    desc: "BLE is requested to slow down",
  }, // Request the BLE to slow down connection parameters to let the app to save config
  ble_reduce_cnx_speed_done: { id: 0x0a0b, desc: "BLE slowed down" }, // BLE connection parameters slowed down
  ble_restore_cnx_speed_rqst: { id: 0x0a0c, desc: "BLE speed restored" }, // Request the BLE to restore connection parameters after the configuration saved
...
export const BUTTON_TIMING_BIT_OFFSETS: Bit_offsets = {
  duration_of_press: {
    desc: "Duration of Press [0.5s]",
    bit_offset: 0,
    bit_length: 4,
    unit: "0.5s",
  },
  duration_diff_of_long_press: {
    desc: "Additional Duration of Long Press [0.5s]",
    bit_offset: 4,
    bit_length: 4,
    unit: "0.5s",
  },
  button1_debounce_time: {
    desc: "Button 1 Debounce Time [ms]",
    bit_offset: 8,
    bit_length: 8,
    unit: "ms",
  },
  button2_debounce_time: {
    desc: "Button 2 Debounce Time [ms]",
    bit_offset: 16,
    bit_length: 8,
    unit: "ms",
  },
...
export const LORAWAN_TX_STRATEGY_BITS: Option_bits = {
  ...
  auto_inc_dr_if_oversized: {
    desc: "Automatically icrease the DR of over-sized messages",
    bit: 3,
  },
...
export function c_string_to_bytes(
  paramValue: string,
  byte_len: number,
): number[] {
  if (paramValue.slice(0, 1) != "{" && paramValue.slice(-1) != "}") {
    throw new Error(
      `Must be a hex encoded byte array. Expected format is "{aa,bb,cc,...}")`,
    );
  }
  ...
  for (let i = 0; i < paramValueHexStringArray.length; i++) {
    if (!REGEXP_HEXBYTE.test(paramValueHexStringArray[i])) {
      throw new Error(
        `Invalid hex encoded byte format. Expected format is "{aa,bb,cc,...}")`,
      );
    }
...
export function hex_to_bytes(hex: string, byte_len: number): number[] {
  ...
  if (!(hex.length === 2 * byte_len && REGEXP_HEXSTRING.test(hex))) {
    throw new Error(
      `must be a valid Hexadecimal String representation of ${byte_len} bytes`,
    );
  }
  const bytes = [];
  for (let i = 0; i < byte_len; i++) {
    bytes.push(parseInt(hex.slice(i * 2, (i + 1) * 2), 16));
  }
  return bytes;
}
...
export function validate_i32_param(
  n: number,
  min: number,
  max: number,
): number {
...
export function validate_options_param(
  i: number,
  options: Option_vals,
): number {
...
export function button_timing_val_to_str(
  val: Button_timing_gui_val,
...
export function button_timing_str_to_val(
  str: Button_timing_gui_str,
): Button_timing_gui_val {
  try {
    return {
      duration_of_press: parse_txt_as_i32(str.duration_of_press).i32_num,
      duration_diff_of_long_press: parse_txt_as_i32(
        str.duration_diff_of_long_press,
      ).i32_num,
      button1_debounce_time: parse_txt_as_i32(str.button1_debounce_time)
        .i32_num,
      button2_debounce_time: parse_txt_as_i32(str.button2_debounce_time)
        .i32_num,
    };
  } catch (err) {
    throw new Error(
...
export function encode_button_timing(gui_val: Button_timing_gui_val): number {
  ...
  if ((gui_val.duration_of_press & 0x0f) === gui_val.duration_of_press) {
    ...
    throw new Error(
      'The value of the "Duration of Press" field must be in the range of 0..15!',
    );
  }

  if (
    (gui_val.duration_diff_of_long_press & 0x0f) ===
    gui_val.duration_diff_of_long_press
  ) {
    param_val |= gui_val.duration_diff_of_long_press << 4;
  } else {
    throw new Error(
      'The value of the "Duration diff of Long Press" field must be in the range of 0..15!',
    );
  }

  if (
    (gui_val.button1_debounce_time & 0xff) ===
    gui_val.button1_debounce_time
  ) {
    param_val |= gui_val.button1_debounce_time << 8;
  } else {
    throw new Error(
      'The value of the "Button #1 Debounce Time" field must be in the range of 0..255!',
    );
  }

  if (
    (gui_val.button2_debounce_time & 0xff) ===
    gui_val.button2_debounce_time
  ) {
    param_val |= gui_val.button2_debounce_time << 16;
  } else {
    throw new Error(
      'The value of the "Button #2 Debounce Time" field must be in the range of 0..255!',
    );
  }
...
export function led_map_val_to_str(
  val: Led_map_gui_val_item[],
): Led_map_gui_str_item[] {
...
export function led_map_str_to_val(
  str: Led_map_gui_str_item[],
): Led_map_gui_val_item[] {
  try {
...
export function encode_led_map(gui_val: Led_map_gui_val_item[]): number[] {
  ...
  for (const item of gui_val) {
    if ((item.count & 0b0011_1111) !== item.count) {
      throw new Error(
        `The values of all count fields must be in the range 0..63`,
      );
    }
...
export function buzzer_map_val_to_str(
  val: Buzzer_map_gui_val_item[],
...
export function buzzer_map_str_to_val(
  str: Buzzer_map_gui_str_item[],
): Buzzer_map_gui_val_item[] {
  try {
...
export function encode_buzzer_map(
  gui_val: Buzzer_map_gui_val_item[],
): number[] {
  const param_val: number[] = [];
...
export function decode_buzzer_map(
  param_val: number[],
): Buzzer_map_gui_val_item[] {
  const gui_val = [];
...
export function encode_geoloc_profile_triggers(
  gui_val: Geoloc_profile_triggers_gui_val,
): number {
  let param_val = 0;
...
export function decode_geoloc_profile_triggers(
  param_val: number,
...
export function encode_geoloc_technologies(
  gui_val: Geoloc_technologies_gui_val_item[],
): number[] {
  const param_val: number[] = [];
...
export function decode_geoloc_technologies(
  param_val: number[],
...
export function cell_psm_time_val_to_str(
  val: Cell_psm_time_gui_val,
...
export function cell_psm_time_str_to_val(
  str: Cell_psm_time_gui_str,
...
export function encode_cell_psm_time(gui_val: Cell_psm_time_gui_val): number {
  ...
  if (
    (gui_val.number_of_time_units & 0b0001_1111) !==
    gui_val.number_of_time_units
  ) {
...
export function decode_cell_psm_time(
  param_val: number,
  time_units: Option_vals,
): Cell_psm_time_gui_val {
  ...
  for (const key in time_units) {
    if (time_units[key].val == time_unit) {
      is_valid_time_unit = true;
    }
  }
...
export function encode_tx_strategy(
  gui_val: Lorawan_s1_tx_strategy_gui_val,
): number {
  let param_val = 0;
...
export function decode_tx_strategy(
  param_val: number,
...
export function validate_search_bands(val: number[], len: number): number[] {
  ...
  for (const network_id of val) {
    ...
    if (!valid) {
      throw new Error(
        `invalid network band ID:0x${network_id.toString(16).padStart(2, "0")} in the search list`,
      );
    }
  }
...
export const CORE_CLI_PASSWORD: Optionally_saved_param_i32 = {
  id: 0x010e,
  name: "core_cli_password",
  title: "CLI password",
  desc: "CLI password",
  type: "i32",
  min_val: 0,
...
export const GEOLOC_GNSS_HOLD_ON_MODE: Param_options = {
  ...
  options: {
    ...
    while_moving_and_gnss_is_used: {
      desc: "When moving and GNSS is used",
      val: 5,
    },
  },
  default_val: 0,
...
export const LR_WIFI_REPORT_NB_BSSID: Param_i32 = {
  ...
  title: "Max num of reported BSSID",
  desc: "Scan is finished once this number of BSSIDs are found",
  type: "i32",
  min_val: 1,
  max_val: 32,
  default_val: 4,
...
export const LR_WIFI_MIN_NB_BSSID: Param_i32 = {
  ...
  title: "Min num of BSSID per scan",
  desc: "Minimum number of BSSIDs to consider the scan as success",
  type: "i32",
  min_val: 0,
  max_val: 32,
  default_val: 3,
...
export const LR_WIFI_MIN_RSSI: Param_i32 = {
  ...
  desc: "Minimum RSSI value to report the BSSID. A null value disables the filter.",
  type: "i32",
  min_val: -200,
  max_val: 0,
  default_val: 0,
...
export const BLE_SCAN1_TYPE: Param_options = {
  ...
  options: {
    ...
    eddystone_uuid_beacons_only: {
      desc: "Eddystone UUID beacons only",
      val: 1,
    },
    ...
    custom_only_based_on_filters: {
      desc: "Custom (only based on filters)",
      val: 6,
    },
...
export const CELL_SEARCH_BANDS: Param_cell_search_bands = {
  ...
  len: 19,
  default_val: [
    0x01, 0x03, 0x08, 0x13, 0x14, 0x1c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
export const CELL_CNX_NW_RECONNECT_TIMEOUT: Param_i32 = {
  id: 0x0a05,
  name: "cell_cnx_nw_reconnect_timeout",
...
export const BLE_CNX_BEHAVIOR: Param_options = {
  ...
  options: {
    ...
    enabled_no_passkey_no_slow_adv: {
      desc: "Enabled - No passkey - No slow adv",
      val: 3,
    },
    enabled_passkey_no_slow_adv: {
      desc: "Enabled - Passkey - No slow adv",
      val: 4,
    },
  },
  default_val: 1,
