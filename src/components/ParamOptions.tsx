...
import { Param_options, parse_txt_as_i32, validate_options_param } from '../abw/params_at3';
import { DefaultIndicator } from "./DefaultIndicator.tsx";


export function ParamOptions({param_const, params_ref, defpar_ref, dummy_state}: {
  ...
  const default_desc = default_desc_key ? param_const.options[default_desc_key].desc : '';
  ...
  return (
    ...
    <div className="inline-block">
      ...
      >
        ...
        {
          (err_str.current !== '') ? <option key={state} value={state}>{state}</option> : ''
        }

        {
          Object.entries(param_const.options).map( ([, option]: [string, {desc: string, val: number}])  => {
            return (
