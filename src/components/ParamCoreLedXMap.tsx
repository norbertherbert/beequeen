...
export function ParamCoreLedXMap({
  param_const,
  params_ref,
  defpar_ref,
  dummy_state,
  ...
  const calculate_initial_state: () => Led_map_gui_str_item[] = () => {
    ...
    if (par === undefined || par === null) {
      ...
      try {
        ...
        const initial_state = led_map_val_to_str(
          decode_led_map(c_string_to_bytes(par, param_const.len)),
        );
  ...
  function handle_change(
    ...
    try {
      ...
      param_str.current = bytes_to_c_string(
        encode_led_map(led_map_str_to_val(new_state)),
        param_const.len,
      );
              ...
              <tr key={line_id}>
                <td>
                  <Select
                    ...
                    sizing="sm"
                  >
                    {Object.entries(LED_PATTERNS).map(
                      ([option_key, option]: [
                        string,
