...
export function ParamGeolocProfileXTriggers({param_const, params_ref, defpar_ref, dummy_state}: {
  ...
  function calculate_initial_state(): Geoloc_profile_triggers_gui_val {
    ...
    if ((par === undefined) || (par === null)) {
      ...
      try {
        ...
        const decoded_parsed_param = decode_geoloc_profile_triggers(parsed_param.i32_num);
  ...
  const [state, set_state] = useState<Geoloc_profile_triggers_gui_val|undefined>()
  ...
  return (
    ...
    <div className="inline-block">
      ...
      <div className="flex max-w-md flex-col gap-0" id="checkbox">
        {
          Object.entries(GEOLOC_PROFILE_TRIGGERS).map( ([key, option]: [string, {desc: string, val: number}])  => {
            return (
