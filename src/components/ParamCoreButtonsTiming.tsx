...
export function ParamCoreButtonsTiming({param_const, params_ref, defpar_ref, dummy_state}: {
  ...
  function handle_change(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const param_i32 = encode_button_timing(button_timing_str_to_val(new_state));
  ...
  const duration_diff_of_long_press_val = parseInt(state.duration_diff_of_long_press);
  isNaN(duration_of_press_val)
  const calculated_duration_of_press = isNaN(duration_of_press_val) ? 'Duration: -' : `Total duration: ${duration_of_press_val}*0.5s = ${duration_of_press_val*0.5}s`;
  const calculated_duration_of_long_press = isNaN(duration_of_press_val) ? 'Total duration: -' : `Total duration: ${duration_of_press_val*0.5}s+${duration_diff_of_long_press_val}*0.5s = ${(duration_of_press_val+duration_diff_of_long_press_val)*0.5}s`;
  ...
  return (
    ...
    <div className="inline-block">
      ...
      <div className="flex max-w-md flex-col gap-0 pt-2" id="button_form_fields">
        {
          Object.entries(BUTTON_TIMING_BIT_OFFSETS).map( ([key, option]: [string, {desc: string, }])  => {
            return (

              <div className="pb-1" key={key}>

                <Label htmlFor={`${param_const.name}_${key}`} value={option.desc} />
                ...
                <span className="text-xs">
                  ...
                  {(key == 'duration_diff_of_long_press') && calculated_duration_of_long_press}
                </span>
