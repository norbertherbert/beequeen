...
export function ParamBytearray({param_const, params_ref, defpar_ref, dummy_state}: {
  ...
  const calculate_initial_state: () => string = () => {
    ...
    if ((par === undefined) || (par === null)) {
      ...
      try {
        ...
        const initial_state =  bytes_to_hex( c_string_to_bytes(param_str.current, param_const.len), param_const.len )
    ...
    if (
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        if (e.key == 'v') {
          ...
          if (selection_end - selection_start > 0 && text.length > selection_end - selection_start) { text = text.slice(0, selection_end - selection_start)}
          if (text) {
            let new_text = gui_val.slice(0, selection_start) + text + gui_val.slice(selection_start + text.length);
            new_text = new_text.slice(0, 2*param_const.len);



            
            param_str.current = bytes_to_c_string( hex_to_bytes(new_text, param_const.len), param_const.len );
            err_str.current = '';            
            set_state(new_text);
            ...
            if (selection_start > 2*param_const.len) selection_start=2*param_const.len
            selection_end = selection_start;
          }
        } else if (e.key == 'c') {
          await navigator.clipboard.writeText(gui_val.slice(selection_start, selection_end));
        } else {
          return;
        }
        ...
        switch (e.key) {
          ...
          default:
            ...
            if (
              ...
              const new_text = gui_val.slice(0, selection_start) + e.key + gui_val.slice(selection_start+1);
        ...
        switch (e.key) {
          case "ArrowLeft":
              if (selection_start > 0) {selection_start--; selection_end = selection_start};
              break;
          case "ArrowRight":
              if (selection_start < 2*param_const.len) {selection_start++; selection_end = selection_start};
              break;
          case "Backspace":
            {
              if (selection_start === selection_end && selection_start > 0) { selection_start--; }
              let new_text = gui_val.slice(0, selection_start) + ''.padStart(selection_end - selection_start, '0') + gui_val.slice(selection_end);
              new_text = new_text.slice(0, 2*param_const.len); 
