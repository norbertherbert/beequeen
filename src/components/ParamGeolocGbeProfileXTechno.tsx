...
export function ParamGeolocGbeProfileXTechno({param_const, params_ref, defpar_ref, dummy_state}: {
  ...
  const calculate_initial_state: () => Geoloc_technologies_gui_val_item[] = () => {
    ...
    if ((par === undefined) || (par === null)) {
      ...
      try {
        ...
        const initial_state =  decode_geoloc_technologies(c_string_to_bytes(par, param_const.len));
  ...
  const [state, set_state] = useState<Geoloc_technologies_gui_val_item[]|undefined>()
  ...
  function handle_change(e: React.ChangeEvent<HTMLSelectElement>, i: number) {
    ...
    try {
      ...
      param_str.current = bytes_to_c_string(encode_geoloc_technologies(new_state), param_const.len);
  ...
  return (
    ...
    <div className="inline-block">
      ...
      { [0,1,2,3,4,5].map( (key) => { return (
        <div className="flex justify-start items-center mb-1" key={key}>
          <Select 
          ...
          >
            {
              Object.entries(GEOLOC_TECHNOLOGIES).map( ([option_key, option]: [string, {desc: string, val: number}])  => {
                return (
          ...
          />
          <Tooltip 
            content={ 'When checked, then the next technology is always triggered regardless of the success of this one.' }
            trigger="click"
          >
            <svg
              className="ml-1 mb-1 w-4 h-4 text-gray-800 dark:text-white"
              aria-hidden="true"
