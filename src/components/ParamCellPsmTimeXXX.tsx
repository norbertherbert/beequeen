...
export function ParamCellPsmTimeXXX({param_const, params_ref, defpar_ref, dummy_state}: {
  ...
  function calculate_initial_state(): Cell_psm_time_gui_str {
    ...
    if ((par === undefined) || (par === null)) {
      ...
      try {
        ...
        const decoded_parsed_param = decode_cell_psm_time(parsed_param.i32_num, param_const.time_units);
  ...
  function handle_change(e: React.ChangeEvent<HTMLInputElement|HTMLSelectElement>) {
    ...
    try {
      ...
      const param_i32 = encode_cell_psm_time(cell_psm_time_str_to_val(new_state));
  ...
  return (
    ...
    <div className="inline-block">
      ...
      <div className="flex justify-start items-center">
        ...
        <span className={state.time_unit == 7 ? 'hidden': 'pl-1 pr-1'} >*</span> 
        ...
        >
          ...
          {
            Object.entries(param_const.time_units).map( ([, option]: [string, {desc: string, val: number}])  => {
              return (
                <option key={option.val} value={option.val}>{option.desc}</option>
              )
            })
          }
